...
async function handleUtilityInteraction(interaction) {
    ...
    try {
        ...
        if (cmd === 'help') {
            const embed = new EmbedBuilder()
                .setColor('#57F287')
                .setTitle('📖 Bantuan Command')
                .addFields(
                    { name: '🛡️ Moderation', value: '`/warn` `/kick` `/ban` `/timeout` `/clear` `/lock`', inline: false },
                    { name: '🏷️ Tags', value: '`/tag create` `/tag list` `/tag show` • Ketik `!tagname` untuk pakai', inline: false },
                    { name: '🎭 Reaction Roles', value: '`/reactionrole add` `/reactionrole list`', inline: false },
                    { name: '📜 Logging', value: '`/logging set` `/logging events` `/logging status`', inline: false },
                    { name: '⭐ Starboard', value: '`/starboard set` `/starboard status`', inline: false },
                    { name: '🚨 Automod', value: '`/automod toggle` `/automod filters` `/automod badwords`', inline: false },
                    { name: '🎁 Autorole & Welcome', value: '`/autorole` `/welcome`', inline: false },
                    { name: '🌾 Farming', value: '`/farming` `/event` `/customevent`', inline: false }
                );
