...
// ⚠️ JANGAN require database di sini — nanti di-require di dalam fungsi
// biar tidak ada circular dependency error
...
const ADMIN_COMMANDS = [
    ...
    new SlashCommandBuilder().setName('slowmode').setDescription('🐢 Set slowmode')
        .setDefaultMemberPermissions(PermissionFlagsBits.ManageChannels)
        .addIntegerOption(o => o.setName('seconds').setDescription('Detik (0=matikan)').setRequired(true).setMinValue(0).setMaxValue(21600))
        .addChannelOption(o => o.setName('channel').setDescription('Channel (default: ini)').setRequired(false))
        .toJSON(),
...
async function handleLock(interaction) {
    ...
    try {
        if (cmd === 'lock') {
            ...
            await ch.permissionOverwrites.edit(everyone, { ViewChannel: false }, { reason: `[${interaction.user.username}] ${reason}` });
        } else if (cmd === 'unlockview') {
            await ch.permissionOverwrites.edit(everyone, { ViewChannel: null }, { reason: `[${interaction.user.username}] ${reason}` });
        }
...
async function handleWelcome(interaction) {
    const db = require('./database'); // ← require di sini biar tidak circular
    const sub = interaction.options.getSubcommand();
