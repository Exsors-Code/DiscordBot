...
// ⚙️ KONFIGURASI
// ==========================================
const LEADERBOARD_UPDATE_INTERVAL = 30000;    // 30 detik
const FARM_PANEL_UPDATE_INTERVAL = 60000;     // 60 detik
const EVENT_ROLE_IDS = ['1408101505008926840'];
const EDIT_THROTTLE_MS = 3000;
const INTERACTION_LOCK_MS = 1500;
const MEMBER_CACHE_TTL = 30 * 60 * 1000;      // 30 menit

const WL_TO_GEMS = 2000;
...
// ==========================================
// 🚀 FIX: getGuildMemberIds — pakai cache (TIDAK FETCH)
// ==========================================
async function getGuildMemberIds(guildId) {
    try {
        const g = client.guilds.cache.get(guildId);
        if (!g) return guildMemberCache.get(guildId) || new Set();
        ...
        if (now - last < MEMBER_CACHE_TTL && guildMemberCache.has(guildId)) {
            return guildMemberCache.get(guildId);
        }

        // Pakai cache Discord.js (otomatis dari intent GuildMembers)
        if (g.members.cache.size > 0) {
            const ids = new Set(g.members.cache.keys());
            guildMemberCache.set(guildId, ids);
            guildMemberCacheTime.set(guildId, now);
            return ids;
        }

        // Fallback (jarang) — fetch sekali dengan timeout, force false
        const members = await g.members.fetch({ time: 30000, force: false }).catch(() => null);
        if (members) {
        ...
        console.error(`⚠️ getGuildMemberIds: ${e.message}`);
        return guildMemberCache.get(guildId) || new Set();
    }
}
...
function mainEmbed(ud) {
    ...
    let questInfo = null;
    try {
        const qd = questMod.getQuestData(ud);
        const totalDone = qd.completed || 0;
        const totalQuests = qd.quests.length;
        const tierMeta = questMod.TIER_META[qd.tier] || questMod.TIER_META.newbie;
        const notClaimed = qd.quests.filter(q => !qd.claimed[q.id] && (qd.progress[q.id] || 0) >= q.target).length;
        questInfo = {
            done: totalDone,
            total: totalQuests,
    ...
    if (questInfo) {
        fields.push({
            name: '📜  Daily Quest',
            value:
                `${questInfo.tierEmoji} **Tier:** ${questInfo.tierName}\n` +
                `✅ **Progress:** **${questInfo.done} / ${questInfo.total}**` +
                (questInfo.notClaimed > 0 ? `\n🎁 **${questInfo.notClaimed} quest siap di-claim!**` : ''),
            inline: false
        });
    }
...
// ==========================================
// 🎨 MAIN BUTTONS — dengan Quest di dropdown
// ==========================================
function mainButtons(ud) {
    ...
    // Cek quest badge
    let questLabel = 'Daily Quest';
    let questDesc = 'Selesaikan quest untuk hadiah harian';
    try {
        const qd = questMod.getQuestData(ud);
        const notClaimed = qd.quests.filter(q => !qd.claimed[q.id] && (qd.progress[q.id] || 0) >= q.target).length;
        if (notClaimed > 0) {
            questLabel = `Daily Quest (${notClaimed} siap!)`;
            questDesc = `🎁 ${notClaimed} quest siap di-claim!`;
        }
    } catch (e) {}
    ...
    const row2 = new ActionRowBuilder().addComponents(
        new StringSelectMenuBuilder()
            ...
            .addOptions(
                ...
                new StringSelectMenuOptionBuilder().setLabel(questLabel).setDescription(questDesc).setValue('menu_quest').setEmoji('📜'),
...
function gachaEmbed(ud) {
    ...
    return new EmbedBuilder()
        ...
        .setDescription(
            ...
            `🎯 **Total roll:** ${ud.totalGachaRolls || 0}\n` +
...
function profileEmbed(ud) {
    ...
    try {
        const qd = questMod.getQuestData(ud);
        const meta = questMod.TIER_META[qd.tier] || questMod.TIER_META.newbie;
        questLine = `${meta.emoji} ${meta.name} · ${qd.completed || 0}/${qd.quests.length}`;
    } catch (e) {}
...
function renderEmbed(ud) {
    switch(ud.currentView) {
        ...
        case 'quest': return questMod.buildQuestEmbed(ud);
...
function renderButtons(ud) {
    switch(ud.currentView) {
        ...
        case 'quest': return questMod.buildQuestButtons(ud);
        default: return mainButtons(ud);
    }
}

// ==========================================
// BREAK LOGIC (dengan Quest tracking)
// ==========================================
function doBreak(ud) {
    ...
    // ===== QUEST TRACKING =====
    try {
        questMod.trackQuest(ud, 'break_blocks', brk);
        questMod.trackQuest(ud, 'gain_gems', tg);
        if (lg > 0) questMod.trackQuest(ud, 'level_up', lg);
    } catch (e) {}
...
function startAutoFarm(userId) {
    ...
    // Quest tracker auto (tiap 1 detik)
    const autoQuestTracker = setInterval(() => {
        const ud = userCache.get(userId);
        if (!ud || !ud.autoFarm) {
            clearInterval(autoQuestTracker);
            return;
        }
        try { questMod.trackQuest(ud, 'play_auto', 1); } catch (e) {}
    }, 1000);
    if (!autoFarmIntervals._questTrackers) autoFarmIntervals._questTrackers = new Map();
    autoFarmIntervals._questTrackers.set(userId, autoQuestTracker);
}
...
function stopAutoFarm(userId) {
    ...
    if (autoFarmIntervals._questTrackers) {
        const qt = autoFarmIntervals._questTrackers.get(userId);
        if (qt) {
            clearInterval(qt);
            autoFarmIntervals._questTrackers.delete(userId);
        }
    }
}
...
client.once('ready', async () => {
    ...
    console.log(`📊 Refresh loop aktif — Leaderboard tiap ${LEADERBOARD_UPDATE_INTERVAL / 1000}s, Farm Panel tiap ${FARM_PANEL_UPDATE_INTERVAL / 1000}s`);
});
...
function rollGacha(ud) {
    ...
    if (selected.type === 'gems') {
        ...
        if (lvUp > 0) {
            result.extra += ` · 🎉 Level ${beforeLvl} → ${ud.level}`;
    ...
    // Quest: win_gacha
    try { questMod.trackQuest(ud, 'win_gacha', 1); } catch (e) {}
