...
// ==========================================
// 🎁 QUEST HELPER — Track + Notify
// ==========================================
async function trackQuestAndNotify(ud, type, amount, channel = null) {
    try {
        const rewards = questMod.trackQuest(ud, type, amount);
        if (rewards.length > 0) {
            db.saveUser(ud);
            ...
            const target = channel || (activeMessages.get(userId)?.channel);
            if (target) {
                try {
                    await target.send(`🎁 <@${userId}> **Quest selesai!**\n${lines.join('\n')}`);
                } catch {}
            }
        }
        return rewards;
...
function startAutoFarm(userId) {
    ...
    // Track quest auto (1 detik = 1 progress per detiknya)
    const autoQuestTracker = setInterval(() => {
        ...
        try {
            questMod.trackQuest(ud, 'play_auto', 1);
        } catch (e) {}
    }, 1000);
    // Simpan reference untuk clear nanti
